...
- [ ] `database.py`: `Database.open(path)` / `open_memory()`; owns io
      backend + pager; `connect()`.
- [ ] `connection.py`: schema load (+ cache), `prepare(sql)`,
      `execute(sql)` convenience, `close()` semantics.
- [ ] `statement.py`: `step()` generator over the VM (row / done),
      iteration + `run()` drivers on top, `reset()`, `finalize()`,
      use-after-finalize errors; positional parameter binding (`?`, `?N`).
- [ ] Lifecycle unit tests: step-reset-restep, iterator equivalence,
      finalize errors, two independent statements on one connection.
- [ ] `docs/api/reference/public-api.md` + `error-hierarchy.md` current.
- [ ] Start `docs/api/explanation/anatomy-of-a-query.md` (capstone; grows
      every later phase — schedule a paragraph per phase close).

## Phase 8

- [ ] Transaction state on connection: autocommit flag, BEGIN/COMMIT/
      ROLLBACK statements routed; write-txn exclusivity vs a second
      connection (single-process rules; document the simplification).
- [ ] Schema-cookie staleness check on prepare (wires `schema.is_stale`).

## Phase 10+

- [ ] `pragma.py`: `table_info`, `index_list`, `page_count`, `journal_mode`
      (read), ledgered in docs.
- [ ] Named parameter binding (`:name`) when the corpus first needs it.

## Phase 12 ride-alongs

- [ ] Non-interactive exec helper for the CLI; `.schema` support queries.
